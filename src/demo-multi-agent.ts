...
import { CodeAgent, createCodeAgent } from './agent.js'
...
async function main() {
    ...
    // 创建主 Agent（用于分析和写作阶段）
    const mainAgent = await createCodeAgent({
        name: 'CodeAgent - 主智能体',
        skillsDir: '.codeagent/skills',
        sandbox: {
...
前端 + AI 的复合能力是当前最稀缺的技能组合，岗位需求大，薪资溢价明显。
  `
    ...
    // 两个实例同时调用 Doubao，不互相阻塞
    const analystAgentInstance = await createCodeAgent({
        name: '分析师智能体',
        skillsDir: '.codeagent/skills',
        sandbox: {
            workspacePath: process.cwd(),
